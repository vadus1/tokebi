module StatefulConcern
  ...
  def validates_cart
    address_id.present? && items.present?
  end
  ...
  def set_default_address
    ...
    if @user_exists && @user_exists.guest?
      user.move_to(@user_exists)
      @user_exists.set_default_to(address)
      user = @user_exists
    elsif user.guest? || user.member?
